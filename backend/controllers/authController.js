...
const login = async (req, res) => {
    try {
        ...
        if(!email || !password) {
            return res.status(400).json(ApiResponse.getResponse(400,'Usuario y contraseña son requeridas', null))
        }
        ...
        if(!usuario) {
            return res.status(401).json( ApiResponse.getResponse(401,'Autenticación fallida', null));
        }
        ...
        if(email === usuario.email && matchPassword) {
            ...
            .status(200).json(ApiResponse.getResponse(200, 'Autenticado correctamente', token));
        } else {
            res.status(401).json( ApiResponse.getResponse(401,'Autenticación fallida', null));
        }
